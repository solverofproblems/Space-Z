...
def reiniciar_jogo():
    ...
    xnave = (largura//2)
    ynave = (altura//2 + 275)
    xasteroide = randint(14, 1024)
    xasteroide2 = randint(14, 1024)
    yasteroide = -30
    yasteroide2 = -30
...
def reiniciar_tudo():
    global reiniciamento, instrucao, start, pontos, carregamento
    reiniciar_jogo()
    py.mixer.music.set_volume(0)
    reiniciamento = 0
    instrucao = True
...
inst = SpriteSheet("instgif.png", 3, 1)
qntframesinst = inst.sprite_count() - 0.1
frameattinst = 0
...
nave = py.image.load('spaceship.png').convert_alpha()
nave_mask = py.mask.from_surface(nave)
nave_rect = nave.get_rect(center=(largura // 2, altura // 2 + 290))
...
# Aqui colocamos novamente a def reiniciar_jogo justamente pois tem chances da nave pegar em algum dos asteroides mesmo após o jogo "acabar"
        ...
        if pontos < tempojogo:    
            tela.blit(texto_formatado, (1170, 675))
